---
name: obh-add-events
description: Use to assess or add an event catalogue to a project's domain — turning writes into emitted facts. Inspects routes/services and proposes dot.notation event names, payloads, and exact in-transaction emit sites (assessment); then guides `forge add events` and wiring the emits (implementation). Assessment-only by default; implements on request.
---
...
4. **Pin the transaction boundary.** For each event, identify the exact write transaction and note where the emit must go **inside** it (same tx/connection as the DB write) so the fact and the state commit atomically via the outbox. Record the precise call-site (file + function + line region). If a handler does multiple writes, decide whether it emits one event or several.
...
Produces the **event catalogue**. Nothing above mutates the repo — a valid stopping point when you only need the survey (and the input several retrofit skills depend on: obh-retrofit-search / analytics / notifications / webhooks and obh-generate-audit-rules all consume events).

## Implementation (only after the catalogue is agreed)
...
7. **Wire the emits.** In each write handler, call `bus.emit('note.created', payload)` inside the tx. Auto-loaded subscribers live in `apps/api/src/bus.d/*` — add a subscriber file per reacting concern (they run in-process; heavy work should enqueue a job rather than block the request). Use `defineX`/`createXClient` grammar from `@obh/events` for the typed event definitions. Then `forge doctor`.

## Output

**Assessment →** the event catalogue: a table of `event.name` → payload shape → emitting file/tx site → intended consumers, flagging any write that currently emits nothing but should. **Implementation →** the `forge add events` (with `--dry-run` note) + `pnpm migrate` steps and a per-handler wiring plan showing where `bus.emit(...)` goes inside each transaction and which `bus.d/*` subscribers to create.
